...
from game import Game, Player, Bot
        ...
        if not game.result:
            ...
            if game.turn == RED:
                ...
                for checker in player2.checkers:
                    ...
                    moves = [move for move in game.board().valid_moves(checker) if move]
                    for move in moves:
                        moves_dict[math.sqrt(move[1] ** 2 + move[0] ** 2) - current_pos] = (checker, move)

                else:
                    min_len = max(moves_dict.keys())
                    checker = moves_dict[min_len][0]

                if checker.target:
                    current_pos = math.sqrt(checker.row ** 2 + checker.col ** 2)
                    if min_len > current_pos:
                        move = moves_dict[min_len][1]
                        game.board().move_checker(checker, move[0], move[1])
                        game.change_turn()
...
#                     all_possible_moves, jumped_moves = game.board().valid_moves(game.selected_checker())
#                     if game.board().get_checker(row, col) != 0 or (row, col) not in all_possible_moves:
#                         game.selected_checker().change_selected(False)
#                         game.change_selected_checker(0)
#                     else:
...
#                         game.selected_checker().change_selected(False)
#                         game.change_selected_checker(0)
#                         if (row, col) not in jumped_moves:
#                             game.change_turn()
